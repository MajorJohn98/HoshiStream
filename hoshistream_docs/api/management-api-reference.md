...
| `GET /api/library/{id}/metadata/search?q=` | Cinemeta candidates for the entry's cleaned name (or `q`): `{query: {title, year?}, candidates: [{imdbId, name, releaseInfo?, poster?}]}` (≤ 20). `409 metadata-disabled` while enrichment is off; `503 metadata-unavailable` when Cinemeta does not answer |
| `POST /api/library/{id}/metadata/apply` | Body `{imdbId}`. Fetches that title and merges it in **replace** mode (owned fields rewritten, viewer-typed fields untouched) → `{status, written[], metadata}`. `404 metadata-not-found` for an unknown id |
| `POST /api/library/{id}/metadata/refresh` | Re-fetch the matched title (replace mode, bypasses the client cache). Without a match it runs a fresh search and auto-accepts when unambiguous, else stores `needs-review` candidates |
| `DELETE /api/library/{id}/metadata` | Unlink: removes only the fields, tags, episodes, and cached artwork enrichment still owns and drops `metadata`; returns the entry |
...
| `episodes` | `{"S:E": {title?, overview?, released?}}` ≤ 500 keys | per-episode display overrides keyed `season:episode`; `title` ≤ 200, `overview` ≤ 2000, `released` an ISO datetime. Blank title falls back to a cleaned filename. PATCH replaces the whole map; `null` or `{}` clears it |
| `ongoing` | boolean | series still airing; emitted as `behaviorHints.hasScheduledVideos: true` so Stremio keeps the show on the Board. `false` on PATCH removes the field |
...
| `metadata` | object | **server-controlled, rejected on POST/PATCH.** Provenance of fetched details: `provider: "cinemeta"`, `imdbId?`, `status` (`matched` \| `needs-review` \| `unavailable` \| `unmatched`), `fetchedAt?`, `query?`, `candidates?` (≤ 5), `owned[]`, `ownedEpisodes[]`, `ownedTags[]`, `artwork?` (`{poster?, background?, logo?}` → `{file, bytes, etag, fetchedAt, sourceUrl}`), `lastError?`. A PATCH that changes an owned value removes it from the owned lists |
...
| `GET /api/diagnostics` | Redacted support bundle (`no-store`): `generatedAt`, `app {release, node, platform, arch, osRelease, uptimeSeconds}`, `torrServer {online, version, settings}` (tuning fields of `POST /settings get` only — Torznab keys, TMDB key and TLS paths are never read), `speed {homeSpeedMbps, current, recent}`, `telemetry` (playback samples), `pointer` status with any drift observation, `disk {volumes, jobs, schedule}`, `library` counts only (no names or sources), and `logs` (last 300 console lines). Every string is passed through redaction: the access token, `POINTER_PUSH_SECRET`, `Authorization`/`Bearer` values, `magnet:` URIs, `token=`-style query values and the home directory (→ `~`). Logs `diagnostics_exported` |
| `GET /api/torrserver/settings` | TorrServer tuning (`no-store`): `current` and `shipped` objects with the six tunables `UploadRateLimit`, `DownloadRateLimit` (KiB/s, `0` = unlimited), `ConnectionsLimit`, `CacheSize` (bytes), `ReaderReadAHead` (%), `TorrentDisconnectTimeout` (s); `shipped` is read from `packaging/torrserver-settings.json`. `suggestion` is `{UploadRateLimit}` (≈10 % of the measured download line, ≥ 64 KiB/s) while the current upload cap still equals the shipped one and a speed test has run, else `null`. 503 `torrserver_unavailable` when TorrServer does not answer |
...
| `PUT /api/library/{id}/disk-copy/policy` | Series rolling window: `{keepAhead?, evictWatched?}` (`keepAhead` 0–50). `keepAhead` archives the next N unwatched episodes after the one just played; `evictWatched` removes watched copies once those are on disk (never the just-played, streaming, or copying file; kept while the drive is offline). Turning `keepAhead` on switches `scope` to `"selected"` and resets inclusion to files already on disk. All-off clears `diskCopy.policy`. Evicted files carry `evictedAt`. → `200` updated entry, `409` unless a series with disk copy enabled |
