...
| `POST /settings` `{action:"get"}` | Effective server settings (`web/api/settings.go` → `settings.BTSets`, `server/settings/btsets.go`) for the diagnostics bundle (`GET /api/diagnostics`) and the tuning form (`GET /api/torrserver/settings`). The public read keeps only tuning fields (`CacheSize`, `ReaderReadAHead`, `PreloadCache`, disk/BT flags, rate and connection limits, `PeersListenPort`, `ResponsiveMode`); `TorznabUrls[].Key`, `TMDBSettings.APIKey`, `SslKey`/`SslCert` and `TorrentsSavePath` are dropped by the schema. Never retried |
| `POST /settings` `{action:"set", sets}` | Apply the six tunables from System → Status (`PUT /api/torrserver/settings`, `POST …/reset`). `sets.SetBTSets` **replaces the whole struct**, so the client re-reads the full struct with a passthrough schema (kept inside `updateSettings`, never returned or logged), merges the edits and posts it back once. TorrServer then persists the struct itself (with `StoreSettingsInJson` the `JsonDB` writes `<config>/settings.json`, the file the supervisor seeds — the add-on does not write it), drops every torrent, and disconnects/reconnects its BitTorrent client (~2 s). The add-on refuses the call while a stream is active. Failsafes at the pinned commit: `CacheSize 0 → 64 MiB`, `ConnectionsLimit 0 → 25`, `TorrentDisconnectTimeout 0 → 30`, `ReaderReadAHead` clamped 5–100. `def` (TorrServer's own defaults) is not used; "Reset to shipped defaults" reads `packaging/torrserver-settings.json` |
