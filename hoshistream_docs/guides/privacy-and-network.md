...
| Title details from Cinemeta (off by default)              | Only when turned on under System → Status: the cleaned title and year of each added entry (or a search you type) go to Stremio's public Cinemeta add-on at `v3-cinemeta.strem.io` (`CINEMETA_URL`), plus the returned IMDb id for detail fetches. Poster/background/logo are downloaded once from the hosts Cinemeta names (usually `images.metahub.space`) and served from this computer afterwards. Logs record route, status, and size — never the title. Turn it off and no request leaves. |
